...
\begin{equation}
	\overline{P_{clu}(x_t)}= \frac{e^{\gamma . cos(x_t, \mu_k)}}{\sum_{k}e^{\gamma . cos(x_t, \mu_k)}}, \mu_k=\frac{1}{|C_k|}\sum_{x_t \in C_k} x_t
\end{equation}
...
\begin{equation}
	P_{clu}^k(x_t^S) = \frac{e^{\gamma . cos(x_t^S, W_k)}}{\sum_{k}e^{\gamma . cos{x_t^S, W_k}}}
\end{equation}
